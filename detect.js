...
var Detect = function(options){
	...
	if(options.installPluginUtility)
		window.dj_utils = new Detect.Utils(this);

	if(options.installTests)
		window.dj_tests = new Detect.Tests(this);
			...
			if(this._Navigator.plugins && this._Navigator.plugins.length > 1){
				...
				if(options.format){
					...
					for(var i = 0; i < this._Navigator.plugins.length; i++){
						...
						output.content.push({name: plugin.name, description: plugin.description, slug: dj_utils.slug(plugin.name)});
					}
				}
			}
